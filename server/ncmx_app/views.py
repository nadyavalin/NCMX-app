...
from .serializers import NCMXNonconformitiesSerializer, NCMXObservationsSerializer, NCMXImprovementsSerializer, NCMXCommentSerializer, NCMXRescheduleCommentSerializer
...
class Nonconformities(APIView):
    def get(self, request):
        is_archived = request.query_params.get('is_archived')
        nonconformities = NCMXNonconformities.objects.all()
        if is_archived is not None:
            try:
                is_archived = is_archived.lower() == 'true'
                nonconformities = nonconformities.filter(is_archived=is_archived)
            except ValueError:
        ...
        serializer = NCMXNonconformitiesSerializer(nonconformities, many=True)
        return Response({"results": serializer.data})

    def post(self, request, num_nonconf=None):
        if num_nonconf is not None:
            # Обработка восстановления
            nonconformity = get_object_or_404(NCMXNonconformities, num_nonconf=num_nonconf)
            nonconformity.is_archived = False
            nonconformity.save()
            serializer = NCMXNonconformitiesSerializer(nonconformity)
            logger.debug(f"Nonconformity restored: {num_nonconf}")
            return Response(serializer.data, status=status.HTTP_200_OK)

        # Обработка создания нового несоответствия
        serializer = NCMXNonconformitiesSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            logger.debug(f"Nonconformity created: {serializer.data}")
    ...
    def patch(self, request, num_nonconf):
        nonconformity = get_object_or_404(NCMXNonconformities, num_nonconf=num_nonconf)
        serializer = NCMXNonconformitiesSerializer(nonconformity, data=request.data, partial=True)
        if serializer.is_valid():
            ...
            serializer.save()
            logger.debug(f"Nonconformity updated: {num_nonconf}, is_archived: {nonconformity.is_archived}")
            return Response(serializer.data, status=status.HTTP_200_OK)
        logger.error(f"Serializer validation errors: {serializer.errors}")
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def delete(self, request, num_nonconf):
        ...
        logger.debug(f"Nonconformity deleted: {num_nonconf}")
    ...
    def get(self, request):
        ...
        if num_nonconf and not content_type:
            try:
                ...
                comments = comments.filter(content_type='nonconformity', object_id=num_nonconf)
            except (ValueError, TypeError):
